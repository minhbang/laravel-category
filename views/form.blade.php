...
@section('content')
    ...
    <div class="form-group">
        <label class="col-xs-3 control-label">{{ __('Parent Category') }}</label>
        <div class="col-xs-9">
            <p class="form-control-static text-primary">{{ $parent_title }}</p>
        </div>
    </div>
    <div class="form-group{{ $errors->has('title') ? ' has-error':'' }}">
        {!! Form::label('label', __('Title'), ['class' => 'col-xs-3 control-label']) !!}
        <div class="col-xs-9">
    ...
    <div class="form-group{{ $errors->has('slug') ? ' has-error':'' }}">
        {!! Form::label('slug', __('Slug'), ['class' => 'col-xs-3 control-label']) !!}
        <div class="col-xs-9">
